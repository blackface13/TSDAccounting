...
{
    public partial class FrmXtraSUDecrementDetail : FrmXtraBaseVoucherDetail, ISUIncrementDecrementView
    {
        #region Presenters
        ...
        public int RefType
        {
            get { return Convert.ToInt32(this.BaseRefTypeId); }
            set { this.BaseRefTypeId = (RefType)value; }
        }
        ...
        {
            get { return (cboCurrency.EditValue ?? "").ToString(); }
            set { cboCurrency.EditValue = value; }
        }
        public decimal ExchangeRate
        {
            get { return Convert.ToDecimal(cboExchangRate.EditValue ?? 0); }
            set { cboExchangRate.EditValue = value; }
        }
        ...
        public FrmXtraSUDecrementDetail()
        {
            InitializeComponent();
        ...
        protected override bool ValidData()
        {
            ...
            else
            {
                ...
                foreach (var suDetail in SUIncrementDecrementDetails)
                {
                    ...
                    decimal quantityOfInventory = _suIncrementDecrementPresenter.GetQuantity(suDetail.InventoryItemId, suDetail.DepartmentId);
                    if (quantityOfInventory < suDetail.Quantity)
                    {
                        var inventoryItem = lstInventoryItems.Where(w => w.InventoryItemId == suDetail.InventoryItemId)?.FirstOrDefault() ?? null;
                        if(inventoryItem != null)
                        {
                            XtraMessageBox.Show("Bạn không được xuất quá số lượng tồn kho của vật tư " + inventoryItem.InventoryItemCode + " là " + Math.Round(quantityOfInventory, int.Parse(DBOptionHelper.NumberDecimalDigits)), ResourceHelper.GetResourceValueByName("ResDetailContent"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        else
                        {
                            XtraMessageBox.Show("Bạn không được xuất quá số lượng tồn kho của vật tư tại dòng " + (i + 1) + " là " + Math.Round(quantityOfInventory, int.Parse(DBOptionHelper.NumberDecimalDigits)), ResourceHelper.GetResourceValueByName("ResDetailContent"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
