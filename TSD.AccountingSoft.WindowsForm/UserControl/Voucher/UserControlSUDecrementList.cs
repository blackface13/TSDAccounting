...
{
    public partial class UserControlSUDecrementList : BaseVoucherListUserControl, ISUIncrementDecrementsView, IInventoryItemsView, IDepartmentsView
    {
        ...
        public UserControlSUDecrementList()
        {
            InitializeComponent();
        ...
        private void UserControlSUDecrementList_Load(object sender, EventArgs e)
        {
        ...
        public IList<SUIncrementDecrementModel> SUIncrementDecrementModels
        {
            set
            {
                var suDecre = new List<SUIncrementDecrementModel>();

                   suDecre = GlobalVariable.CurrencyType == 0 ? value.Where(x => x.CurrencyCode.Trim() == "USD").ToList() : value.Where(x => x.CurrencyCode.Trim() != "USD").ToList();
                bindingSource.DataSource = suDecre;
                gridView.PopulateColumns(suDecre);
        ...
        {
            return new FrmXtraSUDecrementDetail() { FormCaption = ResourceHelper.GetResourceValueByName("ResSUDecrementCaption") };
        }
