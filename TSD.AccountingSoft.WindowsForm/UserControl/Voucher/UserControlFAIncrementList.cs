/***********************************************************************
 * <copyright file="UserControlFAIncrementList.cs" company="BUCA JSC">
...
{
    /// <summary>
    /// class UserControlFAIncrementList
    /// </summary>
    public partial class UserControlFAIncrementList : BaseVoucherListUserControl, IFixedAssetIncrementsView, IFixedAssetsView, IAutoBusinessesView, IProjectsView, IDepartmentsView, IVoucherTypesView
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UserControlFAIncrementList"/> class.
        /// </summary>
        public UserControlFAIncrementList()
        {
            InitializeComponent();
        ...
        public IList<FixedAssetModel> FixedAssets
        {
            set
            {
                try
                {
                    ...
                    _gridLookUpEditFixedAsset.View.PopulateColumns(value);
        ...
        public IList<FixedAssetIncrementModel> FixedAssetIncrements
        {
            set
            {
                var fixedAsset = new List<FixedAssetIncrementModel>();
                fixedAsset = GlobalVariable.CurrencyType == 0 ? value.Where(x => x.CurrencyCode.Trim() == "USD").ToList() : value.Where(x => x.CurrencyCode.Trim() != "USD").ToList();
        ...
        protected override void LoadDataIntoGrid()
        {
            ...
            {
                _fixedAssetIncrementsPresenter.Display((int)RefTypeId, PostedDate);
            }
        }
        ...
        protected override void LoadDataIntoGridDetail(long refId)
        {
            ...
            _fixedAssetIncrementsPresenter.DisplayVoucherDetail(refId);
        ...
        {
            var fixedAsset = _fixedAssetIncrementsPresenter.DisplayFaInDecreamnet(int.Parse(PrimaryKeyValue));
            var faArmortization = _fixedAssetIncrementsPresenter.DisplayFaArmortization(int.Parse(PrimaryKeyValue));
            if (fixedAsset.Count != 0 || faArmortization.Count != 0)
            {
                XtraMessageBox.Show(string.Format(ResourceHelper.GetResourceValueByName("ResDeleteFixedAssetIncrement")), ResourceHelper.GetResourceValueByName("ResDetailContent"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                new FixedAssetIncrementPresenter(null).Delete(int.Parse(PrimaryKeyValue));
            }
        }
        ...
        {
            var frmParam = new FrmXtraPrintVoucherByLot { RefType = RefType.FixedAssetIncrement };
            frmParam.Show();
        }
        ...
        {
            return new FrmXtraFormFAIncrementDetail();
        }
    }
}
